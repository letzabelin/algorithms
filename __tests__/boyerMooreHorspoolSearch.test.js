...
import { boyerMooreHorspoolSearch } from '../src';
...
test('Text does not include pattern', () => {
  const result = boyerMooreHorspoolSearch('Hello, my name is Andrew', 'meow');
  expect(result).toBe(-1);
});

test('Text includes pattern: first', () => {
  const result = boyerMooreHorspoolSearch('Hello, my name is Andrew', 'my');
  expect(result).toBe(7);
});

test('Text includes pattern: second', () => {
  const result = boyerMooreHorspoolSearch(testText, 'meow');
